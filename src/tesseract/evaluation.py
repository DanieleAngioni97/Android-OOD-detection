...
from tesseract import utils as utils, metrics as metrics, temporal
from tesseract.transcendent import *
...
def fit_predict_update_with_ice(clf, X_train, X_tests,
                                y_train, y_tests, t_train, t_tests,
                                fit_function=None, rej_preformance=False, df=None,
    ...
    """
    # debug
    # X_tests, y_tests, t_tests = X_tests[:2], y_tests[:2], t_tests[:2]

    fit_function = clf.fit if fit_function is None else fit_function
    predict_function = (utils.select_prediction_function(clf, labels_only=True)
                        if predict_function is None else predict_function)
    assert hasattr(clf, 'decision_function')
    df = clf.decision_function if df is None else df

    # X_train, X_cal, y_train, y_cal, t_train, t_cal = \
    #     temporal.time_aware_train_test_split(
    #         X_train, y_train, t_train,
    #         train_size=train_size, test_size=cal_size,
    #         granularity=granularity)

    t_train, t_cal, X_train, X_cal, y_train, y_cal = train_test_split(
        t_train, X_train, y_train, test_size=0.34, random_state=3)

    results = {}
    cal_results_dict, p_val_found_thresholds = None, None
    for i, (X_test, y_test, t_test) in tqdm(enumerate(
            zip(X_tests, y_tests, t_tests)), position=0, leave=True):
        ...
        cal_results_dict = train_calibration_ice(
            ...
            y_cal=y_cal,
            fold_index='{}/{}'.format(1, 3)
        ) if cal_results_dict is None else cal_results_dict
        ...
        scores_p_val_cal = package_cred_conf(cred_p_val_cal, None, "cred")

        # p_val_found_thresholds = find_random_search_thresholds(
        #     scores=scores_p_val_cal,
        #     predicted_labels=pred_cal,
        #     groundtruth_labels=groundtruth_cal,
        #     max_metrics="f1_k",
        #     min_metrics="f1_r",
        #     ceiling="reject_total_perc:0.3",
        #     max_samples=1000) if p_val_found_thresholds is None else p_val_found_thresholds

        p_val_found_thresholds = find_random_search_thresholds_with_constraints(
            scores=scores_p_val_cal,
            predicted_labels=pred_cal,
            groundtruth_labels=groundtruth_cal,
            maximise_vals='f1_k:0.90',
            constraint_vals='kept_pos_perc:0.85,kept_neg_perc:0.85',
            max_samples=100000) if p_val_found_thresholds is None else p_val_found_thresholds

        # scores_probas_cal = {'cred': probas_cal}

        # prob_found_thresholds = find_random_search_thresholds(
        ...
        # prob_found_thresholds = find_random_search_thresholds_with_constraints(
        #     scores=scores_probas_cal,
        #     predicted_labels=pred_proba_cal,
        #     groundtruth_labels=groundtruth_cal,
        #     maximise_vals='f1_k:0.90',
        ...
        # probas_test, pred_proba_test = get_svm_probs(clf, X_test)

        ncms_full_test = get_svm_ncms(df, X_test, y_pred)

        p_val_test_dict = compute_p_values_cred_and_conf(
            train_ncms=ncms,
            groundtruth_train=groundtruth,
            test_ncms=ncms_full_test,
            y_test=y_pred)

        # check why the warning happens
        result, kept_indexes = test_with_rejection_keep_masks(
        ...
        results['rejected'].append(len(kept_indexes) - np.count_nonzero(kept_indexes))
        # save necessary data
        if 'transcend' not in results:
            # results['f1_b'] = [result['f1_b']]
            # results['precision_b'] = [result['precision_b']]
            # results['recall_b'] = [result['recall_b']]
            # results['f1_r'] = [result['f1_r']]
            # results['precision_r'] = [result['precision_r']]
            # results['recall_r'] = [result['recall_r']]
            # results['reject_total_perc'] = [result['reject_total_perc']]
            results['transcend'] = [result]
            results['y_tests'] = [y_test]
            results['y_preds'] = [y_pred]
            results['t_tests'] = [t_test]
        else:
            # results['f1_b'].append(result['f1_b'])
            # results['precision_b'].append(result['precision_b'])
            # results['recall_b'].append(result['recall_b'])
            # results['f1_r'].append(result['f1_r'])
            # results['precision_r'].append(result['precision_r'])
            # results['recall_r'].append(result['recall_r'])
            # results['reject_total_perc'].append(result['reject_total_perc'])
            results['transcend'].append(result)
            results['y_tests'].append(y_test)
            results['y_preds'].append(y_pred)
            results['t_tests'].append(t_test)
