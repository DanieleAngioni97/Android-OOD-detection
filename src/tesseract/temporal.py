...
def time_aware_train_test_split(X, y, t, meta, train_size, test_size,
                                granularity, start_date=None):
    """Partition a dataset composed of time-labelled objects.

    Args:
        X (np.ndarray, csr_matrix): Multi-dimensional array of predictors.
    ...
    Returns:
        (np.ndarray, list, np.ndarray, list, np.ndarray, list):
            ...
            Training partition of meta t.
            List of testing partitions of meta t.
    ...
    X_actual, y_actual, t_actual, meta_actual = X[train], y[train], t[train], meta[train]
    ...
    meta_tests = [meta[index_set] for index_set in tests]

    return X_actual, X_tests, y_actual, y_tests, t_actual, t_tests, meta_actual, meta_tests
